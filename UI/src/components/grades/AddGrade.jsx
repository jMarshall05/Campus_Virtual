...
import { createCalificacion } from "../../api/calificacionesService.js";
import Swal from "sweetalert2";

export default function AddGrade({ onClose }) {
    const [loading, setLoading] = useState(false);
    const [form, setForm] = useState({ calificacion: "", comentario: "" });
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            await createCalificacion({ calificacion: parseFloat(form.calificacion), comentario: form.comentario });
            Swal.fire({ title: "Calificación creada", icon: "success", timer: 2000 });
            onClose();
        } catch (error) {
        ...
        <div className="modal-content" style={{ borderRadius: "16px", border: "none" }}>
            <div className="d-flex justify-content-between align-items-center p-3"
                ...
                <div className="d-flex align-items-center gap-2">
                    ...
                    <h5 className="mb-0 fw-bold">Nueva Calificación</h5>
                </div>
